...
For only querying blocks that are included in the winning branch of the chain the branch endpoint can be used, which returns blocks in descending order starting from the leafs of branches of the block chain.
    ...
    def get_block_hashes(self, chain: int, limit: int = None, next: str = None, minheight: int = None, maxheight: int = None):
 ...
 of orphaned blocks.
    ...
    def get_block_hash_branches(self, chain: int, lower:List[str], upper:List[str], limit: int = None, next: int = None, minHeight: int = None, maxHeight: int = None):
