...
class ChainwebP2P():
    ...
    def notification_of_updated_work(self, chainIdBytes: bytes):
        ...
        Args:
            `chainIdBytes` (bytes): The chain id bytes
            byte (32 Chain Id Bytes)
            The chain id of the chain for which new work is available.

        Raises:
            `TypeError`: If chainIdBytes is not bytes.
            `Exception`: If the request fails.
        """                
        _payload = {}
        if not isinstance(chainIdBytes, bytes):
            raise TypeError("chainIdBytes must be bytes")
        
        _endpoint = self.node.endpoint + "/mining/updates"
        _headers = {"Content-type": "application/octet-stream"}
        _data = chainIdBytes
        r = requests.post(_endpoint, params=_payload, headers=_headers, data=json.dumps(_data))
        if r.status_code != 200:
