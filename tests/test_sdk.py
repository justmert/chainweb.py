...
def test_block_hashes_endpoints():
    ...
    p2p_endpoint = P2PBootstrapAPIEndpoint(P2PBootstrapAPIEndpoint.TestnetNode.US1)
    
    cw = BlockHashesEndpoints(p2p_endpoint)
    r1 = cw.get_block_hashes(0, limit=1)
    assert isinstance(r1, dict) and r1 is not None

    r2 = cw.get_block_hashes(0, limit=10, next=r1["next"], minheight=0, maxheight=100)
    assert isinstance(r2, dict) and r2 is not None
    
    cw.set_node_endpoint(P2PBootstrapAPIEndpoint(P2PBootstrapAPIEndpoint.TestnetNode.US1))
    r3 = cw.get_block_hash_branches(0, lower=[r2['items'][0]], upper=[r2['items'][-1]])
    assert isinstance(r3, dict) and r3 is not None


def test_block_header_endpoints():
    from kadenapy.url import P2PBootstrapAPIEndpoint
    from kadenapy.chainweb_p2p.block_header_endpoints import BlockHeaderEndpoints
    ...
    r4 = cw.get_block_headers_by_hash(0, blockHash=example_hash, responseSchema="base64url")
...
def test_block_payload_endpoints():
    from kadenapy.url import P2PBootstrapAPIEndpoint
    from kadenapy.chainweb_p2p.block_payload_endpoints import BlockPayloadEndpoints
    ...
    r5 = cw.get_batch_of_block_payload_with_outputs(0, payloadHashes=[example_hash])
...
def test_mempool_p2p_endpoints():
    ...
    r2 = cw.check_for_pending_transactions_in_the_mempool(chain=1, requestKeys=["MLOST-liM4bLiuBtrjK5L9-4GynKUejcnp-cgKKlxzU"])
    assert r2 is not None

    r3 = cw.lookup_pending_transactions_in_the_mempool(chain=1, requestKeys=["MLOST-liM4bLiuBtrjK5L9-4GynKUejcnp-cgKKlxzU"])
